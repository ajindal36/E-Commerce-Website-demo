...
<body style="background:url(black.jfif); background-size: cover"> <center>
<h1 style="color: white" align="center"><i>Dashboard For
Influencers</i></h1>
...
$inf_email = $row['inf_email']; $inf_genre = $row['inf_genre'];
...
<style> #but { background-color: #4CAF50;
...
$rec_set=mysqli_query($connection,$query) or die(mysqli_error()); echo "<HTML>";
echo "<form method=POST action=\"infdashboard.php\">"; echo "<select name=genre>";
while ($rec=mysqli_fetch_assoc($rec_set))
{
echo "<option value
=".$rec['brand_genre'].">".$rec['brand_genre']."</option>";
} echo "</select>"; echo "<br>";
echo "<input id=\"but\" type=submit name=submit value=\"Find Brands of genre\">"; echo "</form>";
echo "</HTML>";
?>
<?php
...
echo "<tr>"; echo "<td> $brand_name </td>"; echo "<td> $brand_genre </td>"; echo "<td> $brand_email </td>"; echo "<td> $brand_contact</td>";
echo "</tr>";
}
}
?>
